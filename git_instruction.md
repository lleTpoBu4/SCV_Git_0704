...
> Удалённые репозитории представляют собой проекты, сохранённые в интернете или ещё где-то в сети. У вас может быть несколько удалённых репозиториев, каждый из которых может быть доступен для чтения или для чтения-записи.
    
Для загрузки файлов с удалённого хранилища, необходимо выполнить команду:

`git clone <Ссылка на репозиторий в интернете>` 

>Команда копирует файлы из указанного удалённого репозитория в рабочую папку Git. При этом внутри рабочей папки создаётся дирректория идентичная папке из удалённого репозитория (Клон)
    
Для дальнейшей работы с клонированой деррикторией необходимо сделать её рабочей папкой. Для этого нужно выполнить команду:

`cd <имя папки клона>`
    
### Для создания удалённого репозитория или экспорта имеющегося с локального носителя на удалённый тсервер нужно выполнить следующие операции (на примере GitHub):

1. Создать аккаунт на сервисе для хранения/работы с удалённым репозиторием (GitHub.com)
2. Создать новый репозиторий с помощью инструментов сервиса
3. Экспортировать имеющийся репозиторий с локального хранилища на сервер сервиса с помощью команд в терминале:
    
    `git remote add <имя репозитория> <ссылка на репозиторий GitHub>` 
    
    > Команда создания репозитория на удалённом сервере

    `git branch -M main` 
    > Команда переименовывает основную ветку (master) в "main" для соответствия имени 

    `git push -u <имя репозитория> main` 
    > Команда экспортитрует ветку "main" на удалённый сервер

4. После выполнения данных действий можно продолжить работать в локальном репозитории с сохранением коммитов, а для обновления удалённого репозитория до актуальной версии локального достаточно будет выполнить команду `git push`.
5. Если же была проделана работа с сохранением нескольких коммитов непосредственно через онлайн сервис или с другого рабочего места и есть необходимость загрузить актуальную версию репозитория с удалённого сервера в локальное хранилище, тогда необходимо в терминале выполнить команду 
    `git pull`
>При этом происходит слияние веток локального и удалённого репозитория и, в случае возникновения конфликта версий, необходимо будет их разрешить.
   
При работе с чужими репозиториями, изменения в них вносятся через "pull request" (запрос на изменение. Для этого делается следующее:

* Делаем fork (полную копию) существующего репозитория на свой аккаунт в сервисе
* Делаем clone СВОЕЙ версии репозитория на рабочий компьютер
* Создаем новую ветку и в НЕЕ вносим свои изменения
* Фиксируем изменения (делаем коммиты)
* Отправляем свою версию в свой аккаунт GitHub (`git push`)
